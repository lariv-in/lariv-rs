//! Rune sandbox bindings for tax lookup.

use crate::rune_env::{RuneEnvCapability, RuneEnvRegistrar};

/// Registers tax helpers onto the assistant Rune environment.
...
fn register(rune_env: &mut RuneEnvCapability) {
    ...
    rune_env.register_contextual(
        "search_taxes",
        "search_taxes(#{ query: string, limit?: int }) -> #{ results: [#{ id: int, name: string, percentage: string, tax_type: \"levied\"|\"withholding\", account_id: int|null }] }",
        |_ctx| NativeBinding::Function(Arc::new(search_taxes)),
    );
}
...
fn search_taxes(
    ...
    let value = args
        .first()
        .ok_or_else(|| "search_taxes requires an object argument".to_string())?;
    let parsed: SearchArgs = serde_json::from_value(rune_to_json(value)?)
        .map_err(|e| format!("invalid search_taxes arguments: {e}"))?;
    let query = parsed.query.trim().to_string();
    if query.is_empty() {
        return Err("search_taxes requires query".into());
    }
    let limit = trigram::clamp_search_limit(parsed.limit);
    let db = ctx.db.clone();
    let rows = block_on_async(async move {
        trigram::search::<TaxEntity, _>(&db, &[tax::Column::Name], &query, limit).await
    })
    .map_err(|e| e.to_string())?;
    json_to_rune(json!({
        "results": rows.into_iter().map(|t| json!({
            "id": t.id,
            "name": t.name,
            "percentage": decimal::normalize(t.percentage).to_string(),
            "tax_type": t.tax_type.as_str(),
...
mod tests {
    ...
    #[test]
    fn registers_search_taxes_binding() {
        let names = registered_env().all_names();
        assert!(
            names.iter().any(|name| name == "search_taxes"),
            "expected search_taxes in {names:?}"
        );
    }

    #[tokio::test(flavor = "multi_thread")]
    async fn search_taxes_rejects_missing_query() {
        let cap = registered_env();
        let db = sea_orm::DatabaseConnection::default();
        let store: Arc<DynFilestore> = Arc::new(UnimplementedFilestore);
        let env_ctx = test_env_ctx(&db, &store);
        let out = rune_engine::compile_and_run(&cap, &env_ctx, "search_taxes(#{})", &[]).await;
        let error = out
            .get("error")
            .and_then(|v| v.as_str())
            .unwrap_or_default();
