...
//!
//! Every form is CSRF-typed: [`FormCtx::form`] and [`crate::components::form`]
//! take a [`CsrfToken`], and `#[html_form]` structs include a `csrf` field.
//! State-changing submits check the `csrf_token` cookie against the hidden field
//! (see [`csrf`]). GET query parsing via [`from_urlencoded`] does not require a
//! matching token (GET wrappers omit the field so it stays out of query strings).
//!
//! # When to use
//!
...
//! }
//!
//! // Handler: UserForm::from_multipart(multipart, &csrf).await
//! // GET page: UserForm::render_inputs(&FormCtx::form::<UserForm>(CsrfToken::current()))
//! ```
...
pub use csrf::{
    CSRF_COOKIE, CSRF_FIELD, CsrfToken, csrf_hidden_field, csrf_middleware,
    csrf_token_from_headers, current_csrf_token, generate_csrf_token, scope_csrf, verify_csrf,
    verify_form_csrf,
};
...
pub struct FieldSpec {
    ...
    /// CodeMirror language mode for [`widgets::CodeEditor`] (`plaintext`, `markdown`, `html`, `typst`, …).
...
/// Token stored on every `#[html_form]` struct (generated `csrf` field).
pub trait HasCsrf {
    fn csrf(&self) -> &CsrfToken;
}
...
/// [`Self::from_multipart`] on POST. Urlencoded POSTs go through [`HtmlFormBody`],
/// which requires the `csrf_token` cookie to match the hidden field emitted by
/// [`crate::components::form`]. Rendering always goes through [`FormCtx::form`],
/// which requires a [`CsrfToken`].
pub trait HtmlForm: Sized {
    ...
    fn from_multipart(
        ...
        async move {
            ...
            .await?;
            csrf.verify_field(parts.text.get_first(csrf::CSRF_FIELD))?;
...
///
/// Construct only via [`FormCtx::form`] (requires a [`CsrfToken`]) and its
/// [`FormCtxBuilder`].
#[derive(Default)]
...
impl FormCtx<'_> {
    ...
    pub fn form<'a, F: HtmlForm>(csrf: CsrfToken) -> FormCtxBuilder<'a, F> {
        FormCtxBuilder {
            ctx: FormCtx {
                csrf,
                ..FormCtx::default()
...
impl<'a> FormCtx<'a> {
    ...
    /// CSRF token this form was constructed with.
    pub fn csrf(&self) -> &CsrfToken {
        &self.csrf
    }

    /// Hidden CSRF input using [`Self::csrf`].
    pub fn csrf_field(&self) -> Markup {
        csrf::csrf_hidden_field(&self.csrf)
    }
    ...
    pub fn value_of(&self, name: &str) -> &str {
        if name == csrf::CSRF_FIELD {
            return self.csrf.as_str();
        }
